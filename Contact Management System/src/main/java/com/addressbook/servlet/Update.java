...
public class Update extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			int checked_ele = Integer.parseInt(request.getParameter("d"));
			boolean checked_val = Boolean.parseBoolean(request.getParameter("c"));
			ContactDao dao = new ContactDao(ConnectionProvider.getConnection());
			Contact contact1 = dao.getContact(checked_ele);
			contact1.setReview(checked_val);
